...
@interface fbsdkController ()<WKNavigationDelegate, WKScriptMessageHandler>
...
// 在这里处理从HTML5网页中接收到的消息
- (void)receiveMessageFromHTML:(NSDictionary *)message {
    NSUserDefaults *defaults = [NSUserDefaults standardUserDefaults];
    NSDictionary *retrievedDictionary = [defaults objectForKey:@"mylaunchOptions"];
    FBSDKSettings.sharedSettings.appID = message[@"fbid"];
    FBSDKSettings.sharedSettings.clientToken = message[@"token"];
...
- (void)userContentController:(WKUserContentController *)userContentController didReceiveScriptMessage:(WKScriptMessage *)message
{
    ...
    }else if ([message.name isEqualToString:@"setFbid"]) {
        NSDictionary *messageBody = message.body;
        [self receiveMessageFromHTML:messageBody];
    }
}
...
- (UIView *)loadingView
{
    if (!_loadingView) {
        ...
        imageView.image = [UIImage imageNamed:@"6499421950"];
